...
private:
    static void to_positive(uint64_t* buf, const std::size_t n) {
        assert(buf[n-1] & 0x8000'0000'0000'0000);
...
static void karatsuba_calc_x1(uint64_t* x1, const std::size_t num_x1,
    ...
    const bool sign_a = a.lower_minus_upper(da, num_x1);
    const bool sign_b = b.lower_minus_upper(db, num_x1);
...
void gear::karatsuba(uint64_t* out, std::size_t n_out,
    ...
    // out = x0 + x1*R + x2*R^2, where R=2^64.
    ...
    karatsuba(x2, num_x1, a.upper, a.num_upper, b.upper, b.num_upper);
