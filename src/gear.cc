...
std::size_t gear::get_most_significant_active_bit(const std::uint64_t n) {
    if (n == 0)
        return NotFound;

    constexpr std::size_t WidthVector[] = {32, 16, 8, 4, 2, 1};
    std::size_t idx = 0;
    for (const std::size_t width: WidthVector) {
        const int trial_idx = idx + width;
        if (n >= BitMask[trial_idx])
            idx = trial_idx;
    }
    return idx;
}
...
static std::size_t get_first_one_position(const uint64_t* n, const std::size_t num_n) {
    const std::size_t first_idx = gear::get_first_non_zero_index(n, num_n);
    assert(first_idx != gear::NotFound);

    std::size_t pos = (num_n-1) - first_idx;
    const uint64_t first_block = n[first_idx];
    if (first_block > 0)
        pos += gear::get_most_significant_active_bit(first_block) + 1;
    return pos;
}
